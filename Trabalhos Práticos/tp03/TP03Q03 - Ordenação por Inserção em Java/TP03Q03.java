...
class Lista{
    ...
    //para fazer a comparação das datas de lançamento, foi utilizado o método after
    public void inserctionSort(){
        ...
        for (int i = 1; i < n; i++) {
            ...
            int j = i - 1;
            /**
             * se o array[j] for um filme lançado depois do array[i], o laço interno faz o deslocamento de posições até achar
             * a posição para inserir o array[i]
             */
            while ((j >= 0) && (array[j].getRealeaseDate().after(tmp.getRealeaseDate()))) {
