...
public class TP03Q06{
    ...
    public static void main(String[] args) {
        ...
        filmes.quicksort();
        filmes.selectionSort();//desempate
        filmes.mostrar();

        Arq.openWrite("matrícula_quicksort.txt");
...
class Lista{
    ...
    //ordenação pela situação dos filmes
    private void quicksort(int esq, int dir){
        int i = esq, j = dir;
        Filme pivo = array[(dir+esq)/2];
        while (i <= j) {
            while (array[i].getStatus().compareTo(pivo.getStatus()) < 0){
                i++;
                countC++;
            }countC++;
            while (array[j].getStatus().compareTo(pivo.getStatus()) > 0){
                j--;
                countC++;
            }countC++;
    ...
    //ordenação para os casos de empates no campo situação
    public void selectionSort(){
        for(int i = 0; i < (n-1); i++){
            int menor = i;
            for(int j = (i+1); j < n; j++){
                if(array[menor].getStatus().compareTo(array[j].getStatus()) == 0){
    ...
    public void swapSelect(int a, int b){
        Filme tmp = array[a];
        array[a] = array[b];
        array[b] = tmp;
    }
