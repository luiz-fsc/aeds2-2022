...
class Lista{
    ...
    private void intercalar(int esq, int meio, int dir) {
        ...
        for(int j = esq; j <= dir; j++){
            if(pEsq > meio){
                ...
                array[j] = aux[pEsq++];
            }else if(aux[pEsq].getBudget() < aux[pDir].getBudget()){
                array[j] = aux[pEsq++];
                countC++;
            }else{
                array[j] = aux[pDir++];
        ...
        Filme[] arrayEsq = new Filme[nEsq+1];
        Filme[] arrayDir = new Filme[nDir+1];
        ...
        for(iEsq = iDir = 0, i = esq; i <= dir; i++){
            array[i]= (arrayEsq[iEsq].getBudget() <= arrayDir[iDir].getBudget()) ? arrayEsq[iEsq++] : arrayDir[iDir++];
            countM++;
            countC++;
        }*/
    }
    ...
    //ordenação para os casos de empates no campo orcamento
    public void selectionSort(){
        for(int i = 0; i < (n-1); i++){
            int menor = i;
            for(int j = (i+1); j < n; j++){
                if(array[menor].getBudget() == array[j].getBudget()){
